...
-- 문제7.
-- 전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요. length를 order by
  select dept_name
    from departments
...
-- 문제9.
-- 어떤 직책들이 있나요? 중복 없이 이름이 긴 순서대로 출력해 보세요. order by에 length
...
-- 문제10
-- 현재 Engineer 직책의 사원은 총 몇 명입니까? to_date='9999-01-01' 사원 카운트
select count(title)
  from titles
...
-- 문제11
-- 사번이 13250(Zeydy)인 지원의 직책 변경 상황을 시간순으로 출력해보세요. from_date order by
